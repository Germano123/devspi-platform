...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getAdminCommunities = async (userId: string): Promise<Community[]> => {
    ...
    snapshot.forEach(doc => {
      ...
      const isAdmin = data.members?.some(member => member.userId === userId && member.role === 'admin')
      if (isAdmin) {
        communities.push({
          ...data,
          id: doc.id,
  ...
  const getAllEvents = async (): Promise<Event[]> => {
    ...
    for (const eventDoc of querySnapshot.docs) {
      ...
      const interestedSnapshot = await getDocs(collection(db, "events", eventDoc.id, "interested"))
  ...
  const getEvent = async (id: string): Promise<Event | null> => {
    ...
    if (docSnap.exists()) {
      ...
      // Buscar contagem de interessados
      const interestedSnapshot = await getDocs(collection(db, "events", id, "interested"))

      // Buscar contagem de confirmados
      const attendeesSnapshot = await getDocs(collection(db, "events", id, "attendees"))
  ...
  const deleteEvent = async (id: string): Promise<void> => {
    ...
    const deleteInterestedPromises = interestedSnapshot.docs.map((doc) => deleteDoc(doc.ref))
    const deleteAttendeesPromises = attendeesSnapshot.docs.map((doc) => deleteDoc(doc.ref))

    await Promise.all([...deleteInterestedPromises, ...deleteAttendeesPromises])

    // Depois, excluir o evento
    await deleteDoc(doc(db, "events", id))
  }

  const showInterestInEvent = async (userId: string, eventId: string): Promise<void> => {
  ...
  const confirmAttendanceToEvent = async (userId: string, eventId: string): Promise<void> => {
  ...
  const removeInterestFromEvent = async (userId: string, eventId: string): Promise<void> => {
    await deleteDoc(doc(db, "events", eventId, "interested", userId))
  }

  const removeAttendanceFromEvent = async (userId: string, eventId: string): Promise<void> => {
    await deleteDoc(doc(db, "events", eventId, "attendees", userId))
  }

  const getEventInterested = async (eventId: string): Promise<EventParticipant[]> => {
    const interestedSnapshot = await getDocs(collection(db, "events", eventId, "interested"))
    const interested: EventParticipant[] = []

    for (const interestedDoc of interestedSnapshot.docs) {
      const interestedData = interestedDoc.data() as Omit<EventParticipant, "firstName" | "lastName" | "photoURL">
  ...
  const getEventAttendees = async (eventId: string): Promise<EventParticipant[]> => {
    ...
    for (const attendeeDoc of attendeesSnapshot.docs) {
      const attendeeData = attendeeDoc.data() as Omit<EventParticipant, "firstName" | "lastName" | "photoURL">
  ...
  const getContributor = async (userId: string): Promise<Contributor | null> => {
    const docRef = doc(db, "contributors", userId)
    const docSnap = await getDoc(docRef)

    if (docSnap.exists()) {
      const data = docSnap.data() as Omit<Contributor, "id" | "firstName" | "lastName" | "photoURL">
  ...
  const getAllContributors = async (): Promise<Contributor[]> => {
    ...
    for (const contributorDoc of querySnapshot.docs) {
      const contributorData = contributorDoc.data() as Omit<Contributor, "id" | "firstName" | "lastName" | "photoURL">
  ...
  const getAllProjects = async (): Promise<Project[]> => {
    ...
    for (const projectDoc of querySnapshot.docs) {
      ...
      if (!authorName && projectData.authorId) {
        ...
        authorName = userProfile ? `${userProfile.firstName} ${userProfile.lastName}` : "Usuário desconhecido"
      }
  ...
  const updateProject = async (id: string, data: Partial<ProjectData>): Promise<void> => {
