...
export default function WebringPage() {
  ...
  const parseExperienceToYears = (input: string): number => {
    ...
    const months = monthsMatch ? parseFloat(monthsMatch[1].replace(',', '.')) : 0;
  ...
  const formatExperienceTime = (experienceTime: string): string => {
    ...
    if (unit === "anos" || unit === "ano") {
      return amount === "1" ? "1 ano de experiência" : `${amount} anos de experiência`
    } else if (unit === "meses" || unit === "mês") {
      return amount === "1" ? "1 mês de experiência" : `${amount} meses de experiência`
    }
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">Conecte-se com outros desenvolvedores da comunidade de Piauí</p>
      </div>
              ...
              >
                <span className="truncate">
                  {areaFilters.length === 0
                    ? "Áreas de atuação"
                    : `${areaFilters.length} área${areaFilters.length > 1 ? "s" : ""}`}
                </span>
                  ...
                  <div className="space-y-2">
                    {areasDeAtuacao.map((area) => (
                      <div key={area.id} className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor={`filter-${area.id}`} className="cursor-pointer">
                          {area.label}
                        </Label>
                      </div>
                    ))}
        ...
        {isFilterActive && (
          <div className="mt-4 text-sm text-muted-foreground">
            <p>
              ...
              {areaFilters.length > 0 && (
                <span className="ml-1">
                  Áreas: <strong>{areaFilters.map((area) => getAreaAtuacaoLabel(area)).join(", ")}</strong>
                </span>
              )}
              {experienceFilter && (
                <span className="ml-1">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="mb-4 text-sm text-muted-foreground">
            <p>
              <span className="font-medium">Total na plataforma:</span> {totalDevelopers}{" "}
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {displayedProfiles.length === 0 ? (
              ...
              displayedProfiles.map((profile, index) => (
                <div key={profile.id} ref={index === displayedProfiles.length - 1 ? lastProfileRef : null}>
                  <Card className="overflow-hidden h-full">
                      ...
                      <CardDescription>
                        <div className="flex flex-wrap items-center gap-2 mt-1">
                          {profile.areaAtuacao.map((area) => (
                            <Badge key={area} className={getAreaAtuacaoColor(area)}>
                              {getAreaAtuacaoLabel(area)}
                            </Badge>
                          ))}
                        </div>
                        <div className="mt-2">
                          <span className="text-sm">{formatExperienceTime(profile.tempoExperiencia)}</span>
                        </div>
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="pb-2">
                      <div className="text-sm">
                        <p className="mb-1">
                          <strong>Formação:</strong> {getNivelEnsinoLabel(profile.nivelEnsino)}
                        </p>
                      </div>
                    </CardContent>
